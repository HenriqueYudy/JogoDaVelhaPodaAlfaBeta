...
var padrãoDeGanho = {
	r1: '111000000', r2: '000111000', r3: '000000111',  // Linha
	c1: '100100100', c2: '010010010', c3:'001001001',  // Coluna
	d1: '100010001', d2: '001010100' // Diagonal
	};
...
Board.prototype = {
	...
	toBin: function(player) {
        ...
        // , 2));
		return parseInt(this._board.replace(new RegExp('[ ' + otherPlayer(player) + ']', 'g'),'0').
    			    replace(new RegExp(player, 'g'),'1')
                    , 2);
...
function miniMax(board, player, alpha, beta) {
	...
	return { score: bestScore, move: melhorMovimento };
}
...
function veronicaMove() {
	...
	if(tabuleiro.isEmpty()) {
		...
		// se movimento > 1 ent roda via minMax poda alpha beta
...
function gameOver(text) {
	...
	setTimeout(function() { $('.announce').addClass('tada'); }, 100); // Delay needed to animate
